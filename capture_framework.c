...
/* TODO look if this needs to be handled differently,
 * in the end we do not need it anyway, but who knows */
...
int cf_handle_rx_data(kis_capture_handler_t *caph) {
    printf("Hello, cf_handle_rx_data\n");
//    size_t rb_available;
//
...
int cf_handler_remote_server(kis_capture_handler_t *caph) {
    printf("Hello, cf_handler_remote_server   \n");
...
int cf_handler_remote_connect(kis_capture_handler_t *caph) {
    ...
    // TODO what are these for anyway?
    cf_params_interface_t *cpi;
    cf_params_spectrum_t *cps;
    ...
    /* Reset the last ping */
    // TODO should not be necessary in future
    caph->last_ping = time(0);
    ...
    cbret = (*(caph->probe_cb))(caph, 0, caph->cli_sourcedef, msgstr, &uuid, 
            //NULL, &cpi, &cps); // NULL was for KismetExternal__Command
            &cpi, &cps);
    ...
    /* Open connection to RabbitMQ server for capture data */
    conn = amqp_new_connection();

    socket = amqp_tcp_socket_new(conn);
    if (!socket) {
        fprintf(stderr, "FATAL - Could not create socker to remote host '%s:%u': %s\n",
                caph->remote_host, caph->remote_port, strerror(errno));
        return -1;
    }

    /* Open socket to server */
    status = amqp_socket_open(socket, caph->remote_host, caph->remote_port);
    if (status) {
        fprintf(stderr, "FATAL - Could not open socket to remote host '%s:%u': %s\n",
                caph->remote_host, caph->remote_port, strerror(errno));
        return -1;
    }

    /* Login into server */
    amqp_ret = amqp_login(conn, "/", 0, 131072, 0, AMQP_SASL_METHOD_PLAIN,
                                 caph->login_name, caph->login_pw);
    if (amqp_ret.reply_type != AMQP_RESPONSE_NORMAL){
        fprintf(stderr, "FATAL - Could not login on remote host '%s:%u': %s\n",
                caph->remote_host, caph->remote_port, strerror(errno));
        return -1;
    }
    ...
    if (amqp_get_rpc_reply(conn).reply_type != AMQP_RESPONSE_NORMAL){
        fprintf(stderr, "FATAL - Could not open channel on '%s:%u': %s\n",
                caph->remote_host, caph->remote_port, strerror(errno));
        return -1;
    }

    amqp_exchange_declare(conn, 1, amqp_cstring_bytes(caph->cap_exchange), 
                          amqp_cstring_bytes("topic"), 0, 0, 0, 0,
                          amqp_empty_table);

    if (amqp_get_rpc_reply(conn).reply_type != AMQP_RESPONSE_NORMAL){
        fprintf(stderr, "FATAL - Could not declare exchange on '%s:%u': %s\n",
                caph->remote_host, caph->remote_port, strerror(errno));
        return -1;
    }

    caph->capdata_conn = conn;

    fprintf(stderr, "INFO - Connected to '%s:%u'...\n", caph->remote_host, caph->remote_port);

    /* TODO Send log info about new source to RabbitMQ? */
    //cf_send_newsource(caph, uuid);
...
/* TODO much of this is probably not need as we do not need to look for tcp connection
 * ringbuffers or alike, we use a clever system like RabbitMQ instead
 * look into it */
int cf_handler_loop(kis_capture_handler_t *caph) {
    printf("Hello, cf_handler_loop \n");
    fd_set rset, wset;
    int max_fd;
...
/* activate capturing manually, thus without remote command (as we will not get one)
 * TODO disable this, once remote commands are supported */
    /* Callback ret */
    int cbret = -1;
    ...
    if (caph->open_cb == NULL) {
        pthread_mutex_unlock(&(caph->handler_lock));
        printf("source cannot be opened");
        //cf_send_openresp(caph, kds_cmd->seqno,
        //        false, "source cannot be opened", 0, NULL, NULL, NULL);
        cbret = -1;
    } else {
        ...
        //FIXME is this meaningful data?
        cbret = (*(caph->open_cb))(caph,
                caph->seqno, caph->cli_sourcedef,
                msgstr, &dlt, &uuid,
                &interfaceparams, &spectrumparams);
        ...
        printf("INFO - starting capture...\n");

        if (cbret >= 0) {
            cf_handler_launch_capture_thread(caph);
        }
    }
/* end of manual capturing start */
    ...
    while (1) {
        ...
        /* TODO activate something similar if necessary */
        ...
        // TODO support accepting commands from remote source
        if (FD_ISSET(read_fd, &rset)) {
            printf("FD_ISSET(read_fd), cf_handler_loop \n");
        ...
        // TODO support writing to external host (RabbitMQ) for logging
        if (FD_ISSET(write_fd, &wset)) {
            printf("FD_ISSET(write_fd), cf_handler_loop \n");
...
/* TODO We do not need this for RabbitMQ, but every subfunction is using this function
 * think about a clever way to keep old functions and use this one for comm with RabbitMQ */
int cf_send_packet(kis_capture_handler_t *caph, const char *packtype, // e.g. routing key?
...
/* TODO adapt to send message to rabbitmq server */
...
/* TODO adapt to send warning to rabbitmq server */
int cf_send_warning(kis_capture_handler_t *caph, const char *warning) {
    printf("send_warning: %s\n", warning);
...
/* TODO adapt to send error to rabbitmq server */
...
int cf_send_data(kis_capture_handler_t *caph,
    ...
    if (amqp_get_rpc_reply(caph->capdata_conn).reply_type != AMQP_RESPONSE_NORMAL){
        fprintf(stderr, "Error - Could not publish data on '%s:%u' (exchange %s): %s\n",
                caph->remote_host, caph->remote_port, caph->cap_exchange, strerror(errno));
        return -1;
    }
